...
export namespace avtd {
  ...
  type RefType = types | Constructor;
  ...
  export type AvroFieldType = NamedType | EnumType | ArrayType;
...
export function AvroField(options: avtd.AvroFieldType) {
  const f1: MethodDecorator = (target: any, propertyKey: string): void => {};
